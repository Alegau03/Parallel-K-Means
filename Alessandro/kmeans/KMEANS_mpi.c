 ...
 *
 * MPI version (ottimizzata: rimozione sqrt in assegnazione, single sqrt per maxDist)
 *
...
/* 
Function showFileError: stampa un messaggio di errore in base al valore di "error"
*/
...
/*
Function initCentroids: Copia i dati di alcuni punti casuali come centroidi iniziali
*/
...
*/
static inline float euclideanDistance(const float *p, const float *c, int samples)
{
...
int main(int argc, char* argv[])
{
    ...
    if(argc != 7)
    {
        if(rank == 0)
        {
            ...
            fprintf(stderr,"Usage: mpirun -np <procs> ./KMEANS <InputFile> <K> <MaxIter> <Min%%Change> <Threshold> <OutputFile>\n");
        }
    ...
    MPI_Bcast(&maxThreshold, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
    ...
    // Legge i dati (solo rank 0) e broadcast
    if(rank == 0)
    {
    ...
    // Broadcast centroidi iniziali
    MPI_Bcast(centroids, K*samples, MPI_FLOAT, 0, MPI_COMM_WORLD);

    // Suddivisione del lavoro
    int localSize = lines / numProcs;
    int remainder = lines % numProcs;
    int startIndex = (rank < remainder)
                     ? rank*(localSize+1)
                     : remainder*(localSize+1) + (rank - remainder)*localSize;
    int endIndex   = (rank < remainder)
                     ? startIndex + (localSize+1)
                     : startIndex + localSize;
    if(endIndex > lines) endIndex = lines;
    ...
    do
    {
        ...
        if(rank == 0)
        {
            ...
            {
                fprintf(stderr,"Memory allocation error (globalPointsPerClass / globalAuxCentroids).\n");
                MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
            }
        }

        MPI_Reduce(pointsPerClassLocal, (rank==0?globalPointsPerClass:NULL), K,
                   MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(auxCentroidsLocal, (rank==0?globalAuxCentroids:NULL), K*samples,
        ...
        if(rank == 0)
        {
            ...
            // Calcoliamo spostamento max (usiamo euclideanDistance con sqrt, ma solo su centroidi)
            maxDist = 0.0f;
            for(int c=0; c<K; c++)
            {
                float distC = euclideanDistance(&oldCentroids[c*samples],
                                                &globalAuxCentroids[c*samples],
                                                samples);
                if(distC > maxDist) maxDist = distC;
            }
            ...
            // Messaggi di log
            char buffer[128];
            sprintf(buffer, "\n[%d] Changes: %d\tMaxDist: %f", it, globalChanges, maxDist);
            strcat(outputMsg, buffer);

            // Verifica condizione di terminazione
            if((globalChanges <= minChanges) || (it >= maxIterations) || (maxDist <= maxThreshold))
                continueFlag = 0;
    ...
    MPI_Gather(&myNumPoints, 1, MPI_INT,
    ...
    MPI_Gatherv(classMapLocal, myNumPoints, MPI_INT,
                classMapGlobal, recvCounts, displs, MPI_INT,
                0, MPI_COMM_WORLD);
