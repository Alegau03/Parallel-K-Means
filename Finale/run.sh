...
# Argomenti per l'esecuzione dei programmi
NUM_CLUSTER=6
MAX_ITERATIONS=300
MIN_CHANGES=1
THRESHOLD=0.0001
...
run_program() {
    ...
    echo "Esecuzione di $PROGRAM per n iterazioni..."
    # Esegui il programma e salva l'output, il numero di iterazioni è configurabile
    for ((i=1; i<=100; i++)); do
...
run_program "mpirun -np 8 ./kmeans_mpi_omp" "$INPUT_FILE $NUM_CLUSTER $MAX_ITERATIONS $MIN_CHANGES $THRESHOLD $OUTPUT_FILE_MPI_OMP" "$RESULT_MPI_OMP" "$TIMING_MPI_OMP"
echo "##################################################"
echo "Tutte le esecuzioni sono state completate!"

# Questa parte è utile solamente per l'input2D.inp
cd test_files
...
# Visualizzazione dei grafici per input2D.inp
